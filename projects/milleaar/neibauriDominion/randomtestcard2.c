...
void randomizeGame(struct gameState *G, int *kingdomCards, int* handPos){
    ...
    for(int i = 0; i < G->numPlayers; i++) 
    {
        if(i != currentPlayer)
        {
            G->handCount[i] = floor(Random() * 11);//making closer to possibility of having enough cards for discard
            for(int j = 0; j < G->handCount[i]; j++)
            {
                G->hand[i][j] = kingdomCards[ (int) floor(Random() * 10) ];
            }
        }
    }
...
void testMinion(int choice1, int choice2, struct gameState *post, int handPos) {
    ...
    int result = callMinion(choice1, choice2, post, handPos, currentPlayer);
