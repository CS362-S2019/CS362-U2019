...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case mine:
      mine(card, choice1, choice2, choice3, *state, handPos, *bonus);
    ...
    case baron:
      baron(card, choice1, choice2, choice3, *state, handPos, *bonus);
    ...
    case minion:
      minion(card, choice1, choice2, choice3, *state, handPos, *bonus);
    ...
    case tribute:
      tribute(card, choice1, choice2, choice3, *state, handPos, *bonus);

    case ambassador:
      ambassador(card, choice1, choice2, choice3, *state, handPos,*bonus);
...
int minion (int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
    //discard hand
    // Second bug introduced here by making sure the while loop never executes
...
// First bug introduced here by checking to see if they are not equal
  ...
  for (i = 0; i <= 2; i ++){
    // Second bug introduced here by seeing if the card is equal to copper and silver
...
// First bug introduced here by returning 0 instead of -1
  ...
  // Discard trashed card
  // Second bug introduced here by incrementing trash instead of i
  for (i = 0; i < state->handCount[currentPlayer]; trash++)
  {
    if (state->hand[currentPlayer][i] == trash)
    {
      discardCard(i, currentPlayer, state, 0);
      break;
    }
